...
### This code was used to construct the DB for Israeli PM model for the years of 2006-2015, beacuase it used the IMS meteorological data that was available for this period only.
...
maiac=fread("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/MAIAC_data_082016/AQUA/MAIACAAOT_Israel_2007.csv")
...
saveRDS(maiac,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/raw_data/AOD.AQ.2007.RDS")
...
maiac <- as.data.table(maiac)
maiac=filter(maiac,UN>0 & UN<0.04)
...
saveRDS(maiac,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/raw_data/AOD_after_QA.AQ.2007.RDS")

maiac=readRDS("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/raw_data/AOD_after_QA.AQ.2007.RDS")
...
#create date range
days2007 <- data.table(expand.grid(aodid = lu[, unique(aodid)], day = days))
days2007 <- data.table(expand.grid(aodid = maiac[, unique(aodid)], day = days))
days2007$aodid<-as.character(days2007$aodid)

#merge maiac data
maiac$day <- as.Date(maiac$day)
setkey(maiac,aodid,day)
setkey(days2007,aodid,day)
db2007 <- merge(days2007,maiac, all.x = T)

# precentage of NA in the data
sum(is.na(db2007$aod_055))*100/length(db2007$aod_055)
...
db2007[,.(aodid,long_aod,lon,lat_aod,lat)]
db2007[,c("long_aod","lat_aod"):=NULL]
setnames(db2007,"lon","lon_aod")
setnames(db2007,"lat","lat_aod")

# summary(db2007)

#save maiac aod data for 2007
saveRDS(db2007,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/maiac_aod/AQ.AOD.2007.data.rds")
# db2007=readRDS("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/maiac_aod/AQ.AOD.2007.data.rds")

# saving as shapefile only unique id
# db2007=as.data.table(db2007)
# setkey(db2007,aodid)
# maiac_grid=db2007[!duplicated(aodid)]
# coordinates(maiac_grid) = ~ lon_lu + lat_lu
# proj4string(maiac_grid) = "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"
# writeOGR(maiac_grid,dsn="/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/Temp/", layer="db2007_all", driver="ESRI Shapefile")
...
db2007$m=substr(db2007$day,6,7)

# add ndviid to db2007
#join actual NDVI to aod
ndvi=as.data.table(ndvi)
setkey(ndvi, ndviid, m)
setkey(db2007,ndviid, m)
db2007<- merge(db2007, ndvi,all.x = T)

#delete unnecessery columns
db2007[,c("lat_ndvi","long_ndvi","c.y"):=NULL]
...
db2007 <- merge(db2007,hpbl,all.x = T)
...
setkey(db2007,  pblid, day)
db2007 <- merge(db2007, pbl, all.x = T)

db2007[,c("V1.y","lon","lat","time"):=NULL]
db2007=as.data.table(db2007)
summary(db2007$pbl)

# setnames(db2007,"pbl","pbl_08")
setnames(db2007,"hpbl","pbl_11")
...
db2007=as.data.table(db2007)
db2007[,c("V1.y","lon","lat","time","PBLid","V1"):=NULL]

setnames(db2007,"hpbl","pbl_02")
summary(db2007$pbl_02)
...
jointo.pt <- makepointsmatrix(datatable = db2007, 
...
db2007 <- merge(db2007, joinout[,list(day,Temp,aodid)], all.x = T)
head(db2007)
summary(db2007$Temp)
setnames(db2007,"Temp","Temp_H") # Hourly temperature

## Daily Temperature 
Temp <- fread("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/Meteorological_Data/Daily_Data/Daily_Temp_data_IMS_Pollution.csv")
Temp$date<-paste(Temp$Day,Temp$Month,Temp$Year,sep="/")
Temp[, day:=as.Date(strptime(date, "%d/%m/%Y",tz="GMT"))]
Temp[, c := as.numeric(format(day, "%Y")) ]
Temp[,c("Year","Month","Day","date"):=NULL]
...
head(db2007)
summary(db2007$Temp)
setnames(db2007,"Temp","Temp_D") # Daily temperature
...
setkey(db2007,aodid,day)
setkey(joinout,aodid,day)
db2007 <- merge(db2007, joinout[,list(day,WS,aodid)], all.x = T)
head(db2007)
summary(db2007$WS)
setnames(db2007,"WS","WS_H")

###### Add daily WS
WS <- fread("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/Meteorological_Data/Daily_Data/IMS_stn_July16/WS_D.csv")
...
head(db2007)
summary(db2007$WS)
setnames(db2007,"WS","WS_D")

# ## ADD ventilation coefficient
db2007$vc_D=c(db2007$WS_D/(db2007$daily_hpbl*1000))
db2007$vc_H=c(db2007$WS_H/(db2007$pbl_11*1000))
...
setkey(db2007,aodid,day)
setkey(joinout,aodid,day)
db2007 <- merge(db2007, joinout[,list(day,RH,aodid)], all.x = T)
head(db2007)
summary(db2007$RH)
setnames(db2007,"RH","RH_H")

###### Add Daily RH
RH <- fread("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/Meteorological_Data/Daily_Data/IMS_stn_July16/RH_D.csv")
RH$date<-paste(RH$Day,RH$Month,RH$Year,sep="/")
RH[, day:=as.Date(strptime(date, "%d/%m/%Y",tz="GMT"))]
RH[, c := as.numeric(format(day, "%Y")) ]
RH[,c("Year","Month","Day","date"):=NULL]
...
setkey(db2007,aodid,day)
setkey(joinout,aodid,day)
db2007 <- merge(db2007, joinout[,list(day,Rain,aodid)], all.x = T)
head(db2007)
summary(db2007$Rain)
setnames(db2007,"Rain","Rain_H")

## Daily Rain
Rain <- fread("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/Meteorological_Data/Daily_Data/Daily_Rain_Sum_IMS_Pollutants.csv")
Rain$date<-paste(Rain$Day,Rain$Month,Rain$Year,sep="/")
Rain[, day:=as.Date(strptime(date, "%d/%m/%Y",tz="GMT"))]
Rain[, c := as.numeric(format(day, "%Y")) ]
Rain[,c("Year","Month","Day","date"):=NULL]
...
setkey(db2007,aodid,day)
setkey(joinout,aodid,day)
db2007 <- merge(db2007, joinout[,list(day,NO2,aodid)], all.x = T)
head(db2007)
summary(db2007$NO2)
setnames(db2007,"NO2","NO2_H")

###### Add Daily NO2

NO2 <- fread("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/Meteorological_Data/Daily_Data/Pollution_stn_May16/NO2_D.csv")
NO2$date<-paste(NO2$Day,NO2$Month,NO2$Year,sep="/")
NO2[, day:=as.Date(strptime(date, "%d/%m/%Y",tz="GMT"))]
NO2[, c := as.numeric(format(day, "%Y")) ]
NO2[,c("Year","Month","Day","date"):=NULL]
...
setkey(db2007,aodid,day)
setkey(joinout,aodid,day)
db2007 <- merge(db2007, joinout[,list(day,SO2,aodid)], all.x = T)
head(db2007)
summary(db2007$SO2)
setnames(db2007,"SO2","SO2_H")

###### Add Daily SO2

SO2 <- fread("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/Meteorological_Data/Daily_Data/Pollution_stn_May16/SO2_D.csv")
SO2$date<-paste(SO2$Day,SO2$Month,SO2$Year,sep="/")
SO2[, day:=as.Date(strptime(date, "%d/%m/%Y",tz="GMT"))]
SO2[, c := as.numeric(format(day, "%Y")) ]
SO2[,c("Year","Month","Day","date"):=NULL]
...
joinfrom.pt <- makepointsmatrix(datatable = pmall2007, 
                                xvar = "x_stn_ITM", yvar = "y_stn_ITM", idvar = "stn") 

joinout <- nearestbyday(jointo.pts = jointo.pt, joinfrom.pts = joinfrom.pt, 
                        jointo = db2007, joinfrom = pmall2007, 
                        jointovarname = "aodid", joinfromvarname = "stn", 
...
setkey(db2007,aodid,day)
setkey(joinout,aodid,day)
db2007 <- merge(db2007, joinout[,list(day,PM25,aodid)], all.x = T)
head(db2007)
summary(db2007$PM25)

setnames(db2007,"PM25","PM25_H_mean")
...
setnames(db2007,"PM25","PM25_D_closest")
...
setnames(db2007,"PM25","PM25_D_mean")
...
for(i in unique(db2007$day)) {
  ...
  # head(z)
  db2007$pred[db2007$day==i] = z$var1.pred
  # spplot(z, "var1.pred", at = 0:100)
}

setnames(db2007,"pred","PM25_IDW")
...
joinfrom.pt <- makepointsmatrix(datatable = pm_10all2007, 
                                xvar = "x_stn_ITM", yvar = "y_stn_ITM", idvar = "stn") 

joinout <- nearestbyday(jointo.pts = jointo.pt, joinfrom.pts = joinfrom.pt, 
                        jointo = db2007, joinfrom = pm_10all2007, 
                        jointovarname = "aodid", joinfromvarname = "stn", 
...
setkey(db2007,aodid,day)
setkey(joinout,aodid,day)
db2007 <- merge(db2007, joinout[,list(day,PM10,aodid)], all.x = T)
head(db2007)
summary(db2007$PM10)

setnames(db2007,"PM10","PM10_H_mean")
...
setnames(db2007,"PM10","PM10_D_closest")
setnames(db2007,"PM10","PM10_D_mean")
...
setnames(db2007,"pred","PM10_IDW")

#take out uneeded

############################################### Save MOD3 ##########################################
gc()
# saveRDS(db2007,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod3/TR.MAIAC.2007.mod3.rds")
# x1db2007<- readRDS("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod3/TR.PM25.2007.mod3.rds")

saveRDS(db2007,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod3/AQUA/AQ.MAIAC.2007.mod3.rds")
x1db2007<- readRDS("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod3/AQUA/AQ.MAIAC.2007.mod3.rds")

#calculate weights
x1db2007[, m := as.numeric(format(day, "%m")) ]
x1db2007<-x1db2007[,obs:=1]
x1db2007[is.na(aod_055), obs:= 0]
ws.2007<-dplyr::select(x1db2007,obs,Elev,daily_hpbl,m,Temp_D,aodid,day)

#to save memory
gc()

w1 <- glm(obs ~ Elev+Temp_D+daily_hpbl+as.factor(m),family=binomial,data=ws.2007)
ws.2007$prob <- predict(w1 ,type = c("response"))  
ws.2007$wt <- 1/ws.2007$prob
ws.2007$normwt <- ws.2007$wt/mean(ws.2007$wt)
ws.2007[, c("prob", "wt","obs","Elev", "daily_hpbl" , "m","Temp_D"  ) := NULL]
gc()

setkey(x1db2007,aodid,day)
setkey(ws.2007,aodid,day)
x1db2007 <- merge(x1db2007,ws.2007,all.x = T)
x1db2007[,c("m","obs"):=NULL]
# saveRDS(x1db2007,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod3/TR.MAIAC.2007.mod3.rds")
saveRDS(x1db2007,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod3/AQUA/AQ.MAIAC.2007.mod3.rds")
...
#SPLIT the DATA
#create mod 2 file
db2007.m2 <- db2007[!is.na(aod_055)]
#rm db2007
rm(x1db2007)
gc()
...
# db2007.m2=readRDS("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod2/AQUA/Data_buliding_Dec16/AQ.MAIAC.2007.mod2.rds")
...
names=c("daily_hpbl","ndvi","Elev","dis_inventory","Dis_Mroads","road_den","Pop_dens","Dis_Rd1_2012","Dis_Rd2_2012","Dist_Railw",
        ...
        "WS_D","RH_D","Rain_D","NO2_D" ,"pbl_02","pbl_11")

db2007.m2 = db2007.m2 %>% as.data.frame 
scaled = db2007.m2[,names] %>% dplyr::mutate_each(funs(scale))
colnames(scaled) = paste0(colnames(scaled), ".s")
db2007.m2= cbind(db2007.m2, scaled)
names(db2007.m2)
db2007.m2=as.data.table(db2007.m2)

#save mod2
# saveRDS(db2007.m2,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod2/TR.MAIAC.2007.mod2.rds")
saveRDS(db2007.m2,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod2/AQUA/AQ.MAIAC.2007.mod2.rds")
gc()
...
#clear non continous stations
pmall2007<- PM25[c==2007]
setnames(pmall2007,"X.x","x_stn_ITM")
setnames(pmall2007,"Y.x","y_stn_ITM")

# ADD AOD 055 to PM25 mod 1
jointo.pt <- makepointsmatrix(datatable = pmall2007, 
                              xvar = "x_stn_ITM", yvar = "y_stn_ITM", idvar = "stn") 

joinfrom.pt <- makepointsmatrix(datatable = db2007.m2, 
                                xvar = "x_aod_ITM", yvar = "y_aod_ITM", idvar = "aodid") 

joinout <- nearestbyday(jointo.pts = jointo.pt, joinfrom.pts = joinfrom.pt, 
                        jointo = pmall2007 , joinfrom =db2007.m2, 
                        jointovarname = "stn", joinfromvarname = "aodid", 
                        joinprefix = "nearest", valuefield = "aod_055", 
                        knearest = 9, maxdistance = 1500, 
                        nearestmean = TRUE, verbose = T)

setkey(pmall2007,stn,day)
setkey(joinout,stn,day)
PM25.m1 <- merge(pmall2007, joinout, all.x = T)
...
# ADD AOD 047
db2007.m2_s=db2007.m2[, c("aodid","x_aod_ITM","y_aod_ITM","aod_047","day"), with = FALSE]

jointo.pt <- makepointsmatrix(datatable = PM25.m1, 
                              xvar = "x_stn_ITM", yvar = "y_stn_ITM", idvar = "stn") 

joinfrom.pt <- makepointsmatrix(datatable =db2007.m2_s, 
                                xvar = "x_aod_ITM", yvar = "y_aod_ITM", idvar = "aodid") 

joinout <- nearestbyday(jointo.pts = jointo.pt, joinfrom.pts = joinfrom.pt, 
                        jointo = PM25.m1 , joinfrom = db2007.m2_s, 
                        jointovarname = "stn", joinfromvarname = "aodid", 
...
summary(PM25.m1_D)
# Save RDS files
saveRDS(PM25.m1_D,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod1/mod1.AQ.2003_2015.PM25_Daily/mod1.AQ.2007.PM25_Daily.rds")
...
# Save RDS files
saveRDS(PM25.m1_H,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod1/mod1.AQ.2003_2015.PM25_Hourly/mod1.AQ.2007.PM25_Hourly.rds")
# saveRDS(PM25.m1,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod1/mod1.TR.2007.PM25_Daily.rds")
# saveRDS(PM25.m1,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod1/mod1.TR.2007.PM25_Hourly.rds")
...
setkey(pmall2007,stn,day)
setkey(joinout,stn,day)
PM10.m1 <- merge(pmall2007, joinout, all.x = T)
...
joinout <- nearestbyday(jointo.pts = jointo.pt, joinfrom.pts = joinfrom.pt, 
                        jointo = PM10.m1 , joinfrom = db2007.m2_s, 
                        jointovarname = "stn", joinfromvarname = "aodid", 
                        joinprefix = "nearest", valuefield = "aod_047", 
                        knearest = 9, maxdistance = 1500, 
                        nearestmean = TRUE, verbose = T)
...
# Save RDS files
saveRDS(PM10.m1_D,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod1/mod1.AQ.2003_2015.PM10_Daily/mod1.AQ.2007.PM10_Daily.rds")
# saveRDS(PM10.m1_D,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod1/mod1.TR.2007.PM10_Daily.rds")
...
joinfrom.pt <- makepointsmatrix(datatable = db2007_s.m2, 
                                xvar = "x_aod_ITM", yvar = "y_aod_ITM", idvar = "aodid") 

joinout <- nearestbyday(jointo.pts = jointo.pt, joinfrom.pts = joinfrom.pt, 
                        jointo = PM10.m1 , joinfrom = db2007_s.m2, 
                        jointovarname = "stn", joinfromvarname = "aodid", 
                        joinprefix = "nearest", valuefield = "aod_047", 
                        knearest = 9, maxdistance = 1500, 
                        nearestmean = TRUE, verbose = T)
...
# saveRDS(PM10.m1_H,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod1/mod1.TR.2007.PM10_Hourly.rds")
