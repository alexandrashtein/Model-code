# This code converts the MAIAC hdf files first into tiff files and finaly into csv file
...
pol=readOGR(dsn="/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/Qgis/General/IL_Border","israel_borders")
pol = spTransform(pol, "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
...
for(year in 2015) {
  ...
  files = list.files(pattern = "MAIACTAOT.h03v03.*\\.hdf$", recursive = TRUE) # note that MAIACTAOT is for TERRA data and MAIACAAOT is for AQUA data
  ...
  write.csv(
    result,
    paste0("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/MAIAC_data_082016/MAIACTAOT_Israel_", year, ".csv"),
    row.names = FALSE
  )
