...
mod1$month=month(mod1$day)  

mod1=filter(mod1, year==2015)
# mod1=filter(mod1, month==1 | month==2)
mod1=filter(mod1, month==3 | month==4)
...
#R2.m3
mod3bes15_s=as.data.table(mod3bes15_s)
setkey(mod3bes15_s,day,aodid)
setkey(mod1,day,aodid)
mod1 <- merge(mod1,mod3bes15_s[, list(day,aodid,pred.m3,pred.m3.mix)], all.x = T)
m3.fit.all<- summary(lm(PM25~pred.m3,data=mod1))
print(summary(lm(PM25~pred.m3,data=mod1))$r.squared)    
m3.fit.all<- summary(lm(PM25~pred.m3.mix,data=mod1))
print(summary(lm(PM25~pred.m3.mix,data=mod1))$r.squared)    
...
res[res$type=="PM25", 'm3.R2.space'] <-  print(summary(lm(barpm ~ barpred.mix, data=spatialall))$r.squared)
res[res$type=="PM25", 'm3.rmspe.space'] <- print(rmse(residuals(m1.fit.all.spat)))
...
tempoall$delpm <-tempoall$PM25-tempoall$barpred.mix
...
mod3best[,bestpred_ng := pred.m3.mix]
mod3best[!is.na(pred.m2),bestpred_ng := pred.m2]
mod3best[!is.na(pred.m1),bestpred_ng := pred.m1]
...
# cut the data for one bimon only
mod3best$year=year(mod3best$day)
mod3bes15 <- filter(mod3best,year=="2015")
mod3bes15$month=month(mod3bes15$day)
mod3bes15_s <- filter(mod3bes15,month==3 | month==4)

#save for plotting in QGIS- only one bimonth
out <- mod3bes15_s %>% group_by(aodid) %>%
  summarise(lon=mean(lon_aod, na.rm=TRUE), lat =mean(lat_aod, na.rm=TRUE), bestpred=mean(bestpred, na.rm=TRUE),bestpred_ng=mean(bestpred_ng, na.rm=TRUE))
out<-na.omit(out)
out=as.data.table(out)
...
# Plot the PM2.5 Prediction map for all period
tiff(width = 800, height = 960,res=150,filename="/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/results/Maps/mod3best_check_PM25_pred_2003_2015.tiff")
...
# Plot the PM2.5 Prediction map for jan-feb 2015
tiff(width = 800, height = 960,res=150,filename="/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/results/Maps/mod3best_check_PM25_pred_jan_feb_2015.tiff")
ggplot() +  geom_point(data=out, aes(x=lon, y=lat,color=bestpred),alpha=1, size=1)+
  scale_color_gradient("PM2.5 estimation",low="lightblue",high="red")+ ggtitle("Average PM2.5 estimation for Jan-Feb 2015")
dev.off()

# Plot the PM2.5 Prediction map for March-april 2015
tiff(width = 800, height = 960,res=150,filename="/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/results/Maps/mod3best_check_PM25_pred_March_APR_2015.tiff")
ggplot() +  geom_point(data=out, aes(x=lon, y=lat,color=bestpred),alpha=1, size=1)+
  scale_color_gradient("PM2.5 estimation",low="lightblue",high="red")+ ggtitle("Average PM2.5 estimation for Mar-Apr 2015")
dev.off()

# Plot the PM2.5 Prediction map for March-april 2015 without gam
tiff(width = 800, height = 960,res=150,filename="/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/results/Maps/mod3best_check_PM25_pred_March_APR_2015_no_GAM.tiff")
ggplot() +  geom_point(data=out, aes(x=lon, y=lat,color=bestpred_ng),alpha=1, size=1)+
  scale_color_gradient("PM2.5 estimation",low="lightblue",high="red")+ ggtitle("Average PM2.5 estimation for Mar-Apr 2015- no GAM")
dev.off()

out$diff=out$bestpred-out$bestpred_ng

# Plot the PM2.5 Prediction map for jan-feb 2015
tiff(width = 800, height = 960,res=150,filename="/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/results/Maps/mod3best_predict_diff_PM25_pred_jan_feb_2015.tiff")
ggplot() +  geom_point(data=out, aes(x=lon, y=lat,color=diff),alpha=1, size=1)+
  scale_color_gradient("PM2.5 estimation",low="white",high="black")+ ggtitle("Prediction differnces (resid interpolation) for Jan-Feb 2015")
dev.off()

# Plot the PM2.5 Prediction map for March-Apr 2015
tiff(width = 800, height = 960,res=150,filename="/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/results/Maps/mod3best_predict_diff_PM25_pred_march_apr_2015.tiff")
ggplot() +  geom_point(data=out, aes(x=lon, y=lat,color=diff),alpha=1, size=1)+
  scale_color_gradient("PM2.5 estimation",low="white",high="black")+ ggtitle("Prediction differnces (resid interpolation) for Mar-Apr 2015")
dev.off()

## Plot annual histograms
par(mfcol=c(2,1))
hist(out$bestpred)
hist(out$bestpred_ng)

# Plot the PM2.5 Prediction map for jan-feb 2015
tiff(width = 800, height = 960,res=150,filename="/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/results/Maps/mod3best_noGAM_PM25_pred_jan_feb_2015.tiff")
ggplot() +  geom_point(data=out, aes(x=lon, y=lat,color=bestpred_ng),alpha=1, size=1)+
  scale_color_gradient("PM2.5 estimation",low="lightblue",high="red")+ ggtitle("Average PM2.5 estimation for Jan-Feb 2015- no GAM")
dev.off()

# Plot the PM2.5 Prediction map without GAM
tiff(width = 800, height = 960,res=150,filename="/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/results/Maps/mod3best_check_PM25_pred_2003_2015_No_GAM.tiff")
ggplot() +  geom_point(data=out, aes(x=lon, y=lat,color=bestpred_ng),alpha=1, size=1)+
  scale_color_gradient("PM2.5 estimation",low="lightblue",high="red")+ ggtitle("Average PM2.5 estimation for 2003-2015 Without GAM")
dev.off()

## Compute differnces betweem GAM and non GAM prediction
out$diff=out$bestpred-out$bestpred_ng

# Plot the difference between GAM and non GAM prediction
tiff(width = 800, height = 960,res=150,filename="/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/results/Maps/mod3best_differnce_in_estimation.tiff")
ggplot() +  geom_point(data=out, aes(x=lon, y=lat,color=diff),alpha=1, size=1)+
  scale_color_gradient("PM2.5 estimation",low="blue",high="red", breaks=c(-12.32829,0,0.5))+ ggtitle("Average PM2.5 difference estimation for 2003-2015 ")
dev.off()
