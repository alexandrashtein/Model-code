...
# AQUA data
mod1 <-readRDS("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod1/mod1.AQ.2003_2015.PM10_Daily/mod1.AQ.2003_2015.PM10_Daily_Re_Clean.rds")
...
m1.formula <- as.formula(PM10 ~ aod_047+(1+aod_047|day/metreg_IA)
                         # +aod_047*c +aod_047*FS_BS+FS_BS+aod_047*stn_type
                         +Elev.s +ndvi.s+Pop_dens.s +Dist_WB.s+P_Ur.s+road_den.s
                         +NO2_D.s+RH_D.s+daily_hpbl.s
                         # +Dis_Rd1_2012.s+stn_type+
                         # +P_Ag.s+P_In.s+pbl_02.s+Dist_Railw.s+dis_inventory.s+Dis_Rd2_2012.s
                         # +vc_D.s+WS_D.s+O3_D+Temp_D.s+Rain_D.s
                         )
...
# A model recommended by the fitLMER function

# m3.formula <- as.formula(PM10 ~ aod_047+(1+aod_047|day/metreg_IA)
#                          # +aod_047*c +aod_047*FS_BS+FS_BS+stn_type+aod_047*stn_type
#                          #spatial
#                          +Elev.s +ndvi.s +dis_inventory.s+Dist_Railw.s+Dist_WB.s+P_OS.s+P_Ur.s
#                          # +Dis_Rd2_2012.s+P_In.s+P_Ur.s +P_Ag.s+road_den.s+Pop_dens.s
#                          +RH_D.s+NO2_D.s
#                          # +SO2_D.s+pbl_02.s+vc_D.s+WS_D.s+O3_D+Temp_D.s+Rain_D.s
#                          )
# 
# mod3fit <- lmer(m3.formula,data=mod1, REML = F)
# summary(mod3fit)
# AIC(mod1fit,mod3fit)
# AIC(mod2fit,mod3fit)
...
#save MOD1 
saveRDS(mod1,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod1_pred/mod1_Daily.AQ.2003_2015.PM10_clean.predm1_new.rds")
...
# Save AQUA
saveRDS(mod1.cv,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod1_CV/mod1.AQ.2003_2015.PM10_Daily_clean_CV_new.rds")
...
# Save AQUA results
saveRDS(res,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/results/summary_table/2003_2015/results.mod1.AQ.2003_2015.PM10_Daily_clean_new.rds")
...
mod2=mod2[! mod2$aodid %in% bad_aod$aodid,]
...
saveRDS(mod2,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod2_pred/mod2.AQ_Daily_clean.2003_2015.PM10.predm2_new.rds")
...
#check R2 
print(summary(lm(pred.m2~su,data=mod2))$r.squared)
...
#mod 3 (5-8 h)
mod3 <- readRDS("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod3/AQUA/mod3.AQ.2003_2015.PM25_Daily_Re.rds")
...
# change unreasonable values to NA
mod3=mod3[pred.m3.mix<0, pred.m3.mix:=NA]
# mod3=mod3[pred.m3<0, pred.m3:=NA]
mod3=as.data.table(mod3)

saveRDS(mod3,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod3_pred/mod3.PM10.pred.AQ_Daily_clean.2003_2015_new.rds")
...
mod1 <-readRDS("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod1_pred/mod1_Daily.AQ.2003_2015.PM10_clean.predm1_new.rds")
mod1$aodid=paste(formatC(round(mod1$lon_aod,3),format='f',3),formatC(round(mod1$lat_aod,3),format='f',3),sep="-")
...
res[res$type=="PM10", 'm3.I'] <-print(summary(lm(PM10~pred.m3.mix,data=mod1))$coef[1,1])
res[res$type=="PM10", 'm3.Ise'] <-print(summary(lm(PM10~pred.m3.mix,data=mod1))$coef[1,2])
res[res$type=="PM10", 'm3.slope'] <-print(summary(lm(PM10~pred.m3.mix,data=mod1))$coef[2,1])
res[res$type=="PM10", 'm3.slopese'] <-print(summary(lm(PM10~pred.m3.mix,data=mod1))$coef[2,2])
#RMSPE
...
spatialall<-mod1 %>%
  dplyr::group_by(stn) %>%
  dplyr::summarise(barpm = mean(PM10, na.rm=TRUE), barpred = mean(pred.m3.mix, na.rm=TRUE)) 
spatialall=as.data.frame(spatialall)
...
mod1 <-mod1 %>%
  ...
  dplyr::summarise(PM10 = mean(PM10, na.rm=TRUE), pred.m1 = mean(pred.m1, na.rm=TRUE)) 
mod1=as.data.table(mod1)

#----------------> store the best available
mod3best <- mod3[, list(aodid, lon_aod, lat_aod, day, pred.m3.mix)]
...
# save
saveRDS(mod3best,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/Best_pred/bestpred.AQ_Daily_clean.2003_2015.PM10_new.rds")
...
out<-na.omit(out)
nrow(out)
write.csv(out,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/Best_pred/map.bestpred.AQ_Daily_clean.2003_2015.PM10_new.csv")
...
for (i in 1:length(years))
{
  ...
  summarise(lon=mean(lon, na.rm=TRUE), lat =mean(lat, na.rm=TRUE), bestpred=mean(bestpred, na.rm=TRUE))
  out<-na.omit(out)
