...
m2.formula <- as.formula(PM25 ~ aod_047
                         # +aod_047_mean*c +aod_047_mean*FS_BS+FS_BS+stn_type+aod_047_mean*stn_type
                         #spatial
                         +Elev.s +ndvi.s+Dis_Rd1_2012.s+Dist_Railw.s+Pop_dens.s
                         # +dis_inventory.s
                         # +road_den.s
                         # 
                         # +Dist_WB.s
                         ...
                         +P_Ag.s+P_OS.s
                         #temporal
                         +daily_hpbl.s+Temp_D.s+NO2_D.s+pbl_02.s+WS_D.s
                         # +vc_D.s+O3_D+SO2_D.s+RH_D.s+Rain_D.s
                         +(1+aod_047|day/metreg))

full_model <- m1.formula <- as.formula(PM25 ~ aod_047
                                       +stn_type
                                       #spatial
                                       +Elev.s +ndvi.s+Pop_dens.s+Dis_Rd1_2012.s
                                       +dis_inventory.s
...
fit_all <- lmer(full_model,data=mod1)
library(LMERConvenienceFunctions)
bic_best <- fitLMER.fnc(fit_all,ran.effects="(1+aod_047|day/metreg)",method="AIC")
summary(bic_best)
fitLMER <- lmer(bic_best,data=mod1)
AIC(fitLMER,mod2fit)
mod1$pred.m1 <- predict(bic_best)

# bic_best without stn_type
bic_best <- m1.formula <- as.formula(PM25 ~ aod_047
                                       # +stn_type
                                       #spatial
                                       +Elev.s +ndvi.s
                                       # +Pop_dens.s
                                       ...
                                       +P_Ag.s+P_OS.s
                                       #temporal
                                       +daily_hpbl.s+Rain_D.s+NO2_D.s+WS_D.s
                                       # +Temp_D.s+RH_D.s+pbl_02.s
                                       # +SO2_D.s
                                       +(1+aod_047|day/metreg))
print(summary(lm(PM25~pred.m1,data=mod1))$r.squared)

# Compare between different models
mod1fit <- lmer(bic_best,data=mod1)
...
#save MOD1 
 saveRDS(mod1,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod1_pred/mod1_Daily.AQ.2003_2015.PM25_clean.predm1_new.rds")
...
# Save AQUA
saveRDS(mod1.cv,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod1_CV/mod1.AQ.2003_2015.PM25_Daily_clean_CV_new.rds")
...
# Save AQUA results
      saveRDS(res,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/results/summary_table/2003_2015/results.mod1.AQ.2003_2015.PM25_Daily_clean_new.rds")
...
saveRDS(mod2,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod2/AQUA/mod2.AQ.2003_2015.PM25_Daily_Re_Clean.rds")
...
mod2 <- mod2[,c("day","aodid","PM25_D_mean","lon_aod","lat_aod","pred.m2","aod_047","PM25_D_closest","PM25_IDW","year")]
...
# Remove locations with very high predictions
rm=out[out$predm2>35,]

mod2=mod2[!mod2$aodid %in% rm$aodid,]

saveRDS(mod2,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod2_pred/mod2.AQ_Daily_clean.2003_2015.PM25.predm2_new.rds")

# Plot the PM2.5 Prediction map
out=out[out$predm<35,]

ggplot() +  geom_point(data=out, aes(x=x, y=y,color=predm2),alpha=1, size=1)+
  geom_point(data=rm, aes(x=x, y=y),alpha=1, size=1)+
...
mod3=mod3[pred.m3<0, pred.m3:=NA]
mod3=as.data.table(mod3)

saveRDS(mod3,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod3_pred/mod3.pred.AQ_Daily_clean.2003_2015_new.rds")
...
mod1 <-readRDS("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod1_pred/mod1_Daily.AQ.2003_2015.PM25_clean.predm1_new.rds")
mod1$aodid=paste(formatC(round(mod1$lon_aod,3),format='f',3),formatC(round(mod1$lat_aod,3),format='f',3),sep="-")
nrow(mod1)
...
mod1 <-mod1 %>%
  ...
  dplyr::summarise(PM25 = mean(PM25, na.rm=TRUE), pred.m1 = mean(pred.m1, na.rm=TRUE)) 
mod1=as.data.table(mod1)
...
m3.fit.all<- summary(lm(PM25~pred.m3.mix,data=mod1))
# res[res$type=="PM25", 'm3.R2'] <- print(summary(lm(PM25~pred.m3,data=mod1))$r.squared)    
res[res$type=="PM25", 'm3.R2'] <- print(summary(lm(PM25~pred.m3.mix,data=mod1))$r.squared)
res[res$type=="PM25", 'm3.I'] <-print(summary(lm(PM25~pred.m3.mix,data=mod1))$coef[1,1])
res[res$type=="PM25", 'm3.Ise'] <-print(summary(lm(PM25~pred.m3.mix,data=mod1))$coef[1,2])
res[res$type=="PM25", 'm3.slope'] <-print(summary(lm(PM25~pred.m3.mix,data=mod1))$coef[2,1])
# res[res$type=="PM25", 'm3.slope'] <-print(summary(lm(PM25~pred.m3.mix,data=mod1))$coef[2,1])
res[res$type=="PM25", 'm3.slopese'] <-print(summary(lm(PM25~pred.m3.mix,data=mod1))$coef[2,2])
...
spatialall<-mod1 %>%
    dplyr::group_by(stn) %>%
    dplyr::summarise(barpm = mean(PM25, na.rm=TRUE), barpred = mean(pred.m3.mix, na.rm=TRUE)) 
...
tempoall<-left_join(mod1,spatialall)
tempoall$delpm <-tempoall$PM25-tempoall$barpm
tempoall$delpred <-tempoall$pred.m3.mix-tempoall$barpred
mod_temporal <- lm(delpm ~ delpred, data=tempoall)
...
#import mod2
mod2<- readRDS( "/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod2_pred/mod2.AQ_Daily_clean.2003_2015.PM25.predm2_new.rds")
mod2=as.data.table(mod2)
mod2<-mod2[,c("aodid","day","pred.m2"),with=FALSE]

#----------------> store the best available
mod3best <- mod3[, list(aodid, lon_aod, lat_aod, day, pred.m3,pred.m3.mix)]
...
mod3best[,bestpred := pred.m3.mix]
mod3best[!is.na(pred.m2),bestpred := pred.m2]
mod3best[!is.na(pred.m1),bestpred := pred.m1]
summary(mod3best$bestpred)
mod3best<-dplyr::select(mod3best,day,aodid,lon_aod,lat_aod,bestpred,x_aod_ITM,y_aod_ITM)

# Replace NA values by closest prediction

# Join x,y data from mod3
mod3 <- readRDS("/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/mod3/AQUA/mod3.AQ.2003_2015.PM25_Daily.rds")
mod3=as.data.table(mod3)
mod3=mod3[,list(aodid,day,x_aod_ITM,y_aod_ITM)]
setkey(mod3,aodid,day)
setkey(mod3best,aodid,day)
mod3best <- merge(mod3best,mod3)

mod3best_NULL <- mod3best[is.na(bestpred)]
mod3best_NULL$id=mod3best_NULL$aodid
mod3best_NULL$aodid=NULL

mod3best_COM <- mod3best[!is.na(bestpred)]
setnames(mod3best_NULL,"bestpred","pred")

library(aodlur)
jointo.pt = makepointsmatrix(datatable = mod3best_NULL, xvar = "x_aod_ITM", yvar = "y_aod_ITM", idvar = "id") 
joinfrom.pt = makepointsmatrix(datatable = mod3best_COM, xvar = "x_aod_ITM", yvar = "y_aod_ITM", idvar = "aodid")

joinout = nearestbyday(jointo.pts = jointo.pt, joinfrom.pts = joinfrom.pt, 
                       jointo = mod3best_NULL, joinfrom = mod3best_COM, 
                       jointovarname = "id", joinfromvarname = "aodid", 
                       joinprefix = "nearest", valuefield = "bestpred", 
                       knearest = 8, maxdistance = 3000, 
                       nearestmean = TRUE, verbose = TRUE)

setnames(mod3best_NULL,'id',"aodid")
setnames(joinout,'id',"aodid")
setkey(mod3best_NULL, aodid, day)
setkey(joinout, aodid, day)

mod3best_NULL = merge(mod3best_NULL, joinout,all.x = TRUE)
summary(mod3best_NULL)

mod3best_NULL=mod3best_NULL[,list(aodid,day,lon_aod.x,lat_aod.x,x_aod_ITM.x,y_aod_ITM.x,nearestmean)]
setnames(mod3best_NULL,"lon_aod.x","lon_aod")
setnames(mod3best_NULL,"lat_aod.x","lat_aod")
setnames(mod3best_NULL,"y_aod_ITM.x","y_aod_ITM")
setnames(mod3best_NULL,"x_aod_ITM.x","x_aod_ITM")

mod3best_NULL[is.na(nearestmean) , nearestmean  := 0.1]
setnames(mod3best_NULL,"nearestmean","bestpred")
mod3best=rbind(mod3best_COM,mod3best_NULL)
summary(mod3best$bestpred)
mod3best<-dplyr::select(mod3best,day,aodid,lon_aod,lat_aod,bestpred)

# Check duplicates
temp=dplyr::select(mod3best,aodid,day,bestpred)
any(duplicated(temp)==TRUE)
dup=temp[duplicated(temp)==TRUE]
temp <- temp[temp$aodid %in% dup$aodid & temp$day %in% dup$day,]
temp[order(-rank(day), aodid)]

# Make sure that there is reasonable amount of observations
length(unique(mod3$aodid))*(3*366+10*365) == nrow(mod3best)

# Remove duplicates if needed
mod3best <-mod3best %>%
  dplyr::group_by(aodid,day) %>%
  dplyr::summarise(bestpred = mean(bestpred, na.rm=TRUE), lon_aod = mean(lon_aod, na.rm=TRUE),lat_aod = mean(lat_aod, na.rm=TRUE))

#save
saveRDS(mod3best,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/Best_pred/bestpred.AQ_Daily_clean.2003_2015.PM25_new.rds")
saveRDS(mod3best,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/Best_pred/bestpred.no_GAM.AQ_Daily_clean.2003_2015.PM25_new.rds")
...
write.csv(res,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/results/summary_table/2003_2015/results.mod1.AQ.2003_2015.PM25_Daily_clean_new.csv")
write.csv(res,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/results/summary_table/2003_2015/results.mod1.AQ.2003_2015.PM25_Daily_clean_no_GAM_new.csv")
...
out<-na.omit(out)
out=as.data.table(out)
write.csv(out,"/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/work/RDS_files/Best_pred/map.bestpred.AQ_Daily_clean.2003_2015.PM25_no_GAM_new.csv")

# Plot the PM2.5 Prediction map
tiff(width = 800, height = 960,res=150,filename="/media/qnap/Projects/P028.IL.Israel.MAIAC.PM.V2/results/Maps/mod3bet_PM25_pred_2003_2015_new.tiff")
